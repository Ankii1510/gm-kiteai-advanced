...
import WalletModal from "./components/WalletModal"; // modal UI (safe, asset-free)
import logo from "./assets/kite-logo.png"; // if missing it's okay, will not crash

// WalletConnect provider
import EthereumProvider from "@walletconnect/ethereum-provider";

// Config
const WALLETCONNECT_PROJECT_ID = "d6703c69a02c0a568be9c2343cee7c4a"; // <- your project id
const WC_CHAIN_ID = 80001; // <- change this if Kite testnet uses a different chain id
...
function Avatar({ address, size = 40 }) {
  ...
  const style = {
    width: size,
    height: size,
    background: `linear-gradient(135deg, hsl(${hue} 65% 50%), hsl(${(hue + 60) % 360} 65% 40%))`,
  };
  return (
    <div style={style} className="rounded-full flex items-center justify-center text-white font-bold">
      {address ? address.slice(2, 4).toUpperCase() : "GM"}
    </div>
  );
}
...
export default function App() {
  ...
  const [lastSentBy, setLastSentBy] = useState({}); // address -> unix timestamp
  const [txState, setTxState] = useState(null);
  const pollRef = useRef(null);

  // WalletConnect provider instance (for disconnect)
  ...
  // initial provider + read-only contract
  useEffect(() => {
    try {
      ...
      console.error("Failed to create provider/contract:", err);
    }
  }, []);

  // fetch events and build lastSentBy map
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        for (const ev of parsed) {
          const a = String(ev.sender).toLowerCase();
          map[a] = Math.max(map[a] || 0, Number(ev.timestamp));
        }
        setLastSentBy(map);

        // if account already connected, update personal ts
        if (account) {
          const my = String(account).toLowerCase();
          if (map[my]) setLastYourTs(map[my]);
        }
      } catch (err) {
  ...
  // countdown timer for user's cooldown
  useEffect(() => {
  ...
  // connect using WalletConnect v2 (projectId)
  const connectWithWalletConnect = async () => {
    try {
      ...
      // IMPORTANT: call connect() before request() to avoid the "Please call connect() before request()" error
      ...
      // request accounts (should be allowed after connect)
      await wcProvider.request({ method: "eth_requestAccounts" });

      // use ethers with this provider
      const wp = new ethers.BrowserProvider(wcProvider);
      ...
      // attach contract event listener safely
      const connectedAddr = String(addr).toLowerCase();
      cw.on("GMSent", (sender, message, timestamp, event) => {
        try {
          ...
          setLastSentBy((prev) => {
            const next = { ...(prev || {}) };
            next[String(sender).toLowerCase()] = Number(timestamp);
            return next;
          });

          setGlobalCount((c) => c + 1);

          if (String(sender).toLowerCase() === connectedAddr) {
            setYourCount((c) => c + 1);
            setLastYourTs(Number(timestamp));
          }
        } catch (e) {
          console.warn("GMSent handler error", e);
        }
      });
      ...
      // session updates
      wcProvider.on && wcProvider.on("session_update", (args) => {
        console.log("wc session_update", args);
      });
    } catch (err) {
      console.error("WalletConnect error:", err);
      alert("Failed to connect via WalletConnect: " + (err?.message || String(err)));
    }
  };
  ...
  const disconnectWalletConnect = async () => {
    try {
      ...
      console.warn("Error disconnecting wc provider:", err);
    }
    cleanupConnection();
  };
  ...
  // wallet modal choice handler (added walletconnect option)
  async function handleWalletChoice(choice) {
    ...
    if (choice !== "metamask") {
      alert("Unknown wallet choice. Currently supported: metamask, walletconnect.");
      return;
    }

    if (!window.ethereum) {
      return alert("MetaMask not found. Install MetaMask extension and try again.");
    }
    ...
    try {
      ...
      // attach event listener safely: use account from closure variable connectedAddr
      const connectedAddr = String(addr).toLowerCase();
      ...
      cw.on("GMSent", (sender, message, timestamp, event) => {
        try {
          ...
          // update per-address lastSent map
          setLastSentBy((prev) => {
            const next = { ...(prev || {}) };
            next[String(sender).toLowerCase()] = Number(timestamp);
            return next;
          ...
          // compare to connectedAddr (stable)
          if (String(sender).toLowerCase() === connectedAddr) {
            setYourCount((c) => c + 1);
            setLastYourTs(Number(timestamp));
          }
      ...
      // reload if user changes account/network to keep state consistent
  ...
  // connect button (legacy / quick connect for MetaMask)
  const connectWalletQuick = async () => {
    setWalletModalOpen(true);
  };

  // send GM with cooldown enforcement using lastSentBy for per-address check
  const sendGM = async () => {
    if (!contractWrite || !account) return alert("Connect wallet first.");

    const lc = String(account).toLowerCase();
    const lastTs = lastSentBy[lc] || 0;
    const nextAllowed = lastTs + COOLDOWN_SECONDS;
    if (nowUnix() < nextAllowed) {
      const wait = nextAllowed - nowUnix();
      alert(`You already sent a GM. Wait ${formatCountdown(wait)} before sending again.`);
      return;
    }

    // double-check personal lastYourTs
    if (lastYourTs) {
      const nextAllowed2 = lastYourTs + COOLDOWN_SECONDS;
      if (nowUnix() < nextAllowed2) {
        return alert("You are on cooldown.");
      }
    }
    ...
    try {
      ...
      const ts = nowUnix();
      setLastYourTs(ts);
      setLastSentBy((prev) => ({ ...(prev || {}), [lc]: ts }));

      setTimeout(() => setTxState(null), 6000);
    } catch (err) {
      console.error("sendGM error:", err);
  ...
  useEffect(() => {
    // cleanup on unmount: disconnect any active WalletConnect session
    return () => {
      if (wcProviderInstance && wcProviderInstance.disconnect) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-hero text-slate-100">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            {account ? (
              ...
              <button onClick={connectWalletQuick} className="bg-cyan-400 px-4 py-2 rounded-lg font-semibold shadow-xl">Connect Wallet</button>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          <div className="flex items-center justify-center gap-4 mb-4">
            {logo ? <img src={logo} alt="kite" className="w-16 h-16 animate-pulse-slow drop-shadow-lg" /> : <div style={{width:64,height:64}} />}
      ...
      <WalletModal open={walletModalOpen} onClose={() => setWalletModalOpen(false)} onChoose={handleWalletChoice} />
    </div>
  );
}
